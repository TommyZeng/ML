...
import sys
sys.path.append('../初始化、正则化、梯度校验/')
import MNN as mn
import grad_check_testCase as testCase
...
def gradient_check_n(parameters,gradients,X,Y,epsilon=1e-7):
    ...
    """
    #初始化参数
    parameters_values , keys = gc_utils.dictionary_to_vector(parameters) #keys用不到, parameters_values d、w、b参数值集合, keys为名称集合
    grad = gc_utils.gradients_to_vector(gradients) # grad为梯度值集合
    ...
    for i in range(num_parameters):
        ...
        tt, mm = forward_propagation_n(X,Y,gc_utils.vector_to_dictionary(thetaplus))
